...
describe('resolveConfig – Priorität und Pflichtfelder', () => {
  it('CLI schlägt Env; Default-DB, wenn nichts gesetzt', () => {
    ...
    const cfg = resolveConfig(['--out', '/cli', '--days', '30'], { NANOBOT_OUT: '/env' } as any);
  ...
  it('nutzt NANOBOT_OUT, wenn kein --out übergeben wird', () => {
    const cfg = resolveConfig([], { NANOBOT_OUT: '/env' } as any);
    expect(cfg.out).toBe('/env');
    expect(cfg.days).toBe(90); // Default
  });

  it('meldet fehlenden Zielordner als Fehler', () => {
    expect(() => resolveConfig([], {} as any)).toThrow(/Zielordner fehlt/);
  });

  it('lehnt ungültige --days ab', () => {
    expect(() => resolveConfig(['--out', '/x', '--days', '0'], {} as any)).toThrow(/positive Ganzzahl/);
    expect(() => resolveConfig(['--out', '/x', '--days', 'abc'], {} as any)).toThrow(/positive Ganzzahl/);
  });
});
